...
public class Vector3 implements Cloneable, IVector3 {
	...
	/**
	 * Get a relative Vector3 based on the rotationYaw and rotationPitch. Note
	 * that ROTATION PTICH is inversed because of the way Minecraft handles
	 * things in the world.
	 * 
	 * @param rotationYaw
	 *            - Degree
	...
	/** Converts this Vector3m into a Vector2 by dropping the Y axis. */
	...
	public List<Entity> getEntitiesWithin(World worldObj, Class<? extends Entity> par1Class) {
		return worldObj.getEntitiesWithinAABB(par1Class,
				AxisAlignedBB.getBoundingBox(this.intX(), this.intY(), this.intZ(), this.intX() + 1, this.intY() + 1, this.intZ() + 1));
	}
	...
	public void rotate(EulerAngle angle) {
		...
		this.x = x * Math.cos(yawRadians) * Math.cos(pitchRadians) + z
				* (Math.cos(yawRadians) * Math.sin(pitchRadians) * Math.sin(rollRadians) - Math.sin(yawRadians) * Math.cos(rollRadians)) + y
		...
		this.z = x * Math.sin(yawRadians) * Math.cos(pitchRadians) + z
				* (Math.sin(yawRadians) * Math.sin(pitchRadians) * Math.sin(rollRadians) + Math.cos(yawRadians) * Math.cos(rollRadians)) + y
	...
	public MovingObjectPosition rayTraceEntities(World world, Vector3 target) {
		...
		// look.xCoord * reachDistance, startingPosition.yCoord + look.yCoord *
		// reachDistance, startingPosition.zCoord + look.zCoord *
		// reachDistance);

		double checkBorder = 1.1 * reachDistance;
		AxisAlignedBB boxToScan = AxisAlignedBB.getAABBPool().getAABB(-checkBorder, -checkBorder, -checkBorder, checkBorder, checkBorder, checkBorder)
